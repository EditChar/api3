document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderTestSets(tests) {
        ...
        let html = '<ul>';
        tests.forEach(test => {
            const isVisible = test.is_visible !== false; // Varsayılan olarak görünür (true veya undefined)
            const hiddenClass = !isVisible ? 'test-hidden' : '';
            const toggleButtonText = isVisible ? 'Gizle' : 'Aktif Et';
            const toggleButtonClass = isVisible ? 'warning-btn' : 'success-btn';

            html += `
                <li data-testid="${test.id}" class="${hiddenClass}">
                    ...
                    <div>
                        ...
                        <button class="button-style ${toggleButtonClass} toggle-visibility-btn" data-testid="${test.id}">${toggleButtonText}</button>
    ...
    // --- Test Seti Görünürlüğünü Değiştir ---
    async function toggleTestVisibility(testId) {
        if (!testId) {
            alert('Test ID bulunamadı.');
            return;
        }
        try {
            await fetchAPI(`/api/tests/${testId}/toggle-visibility`, {
                method: 'PATCH',
            });
            // alert('Test görünürlüğü başarıyla güncellendi!'); // İsteğe bağlı
            loadTestSets(); // Listeyi yenile
        } catch (error) {
            console.error('Test görünürlüğü güncellenirken hata:', error);
            alert(`Test görünürlüğü güncellenemedi: ${error.message}`);
        }
    }
    ...
    function addAnswerOption() {
        ...
        newAnswerDiv.innerHTML = `
            ...
            <input type="number" name="score_${answerOptionCount}" placeholder="Puan (1-10)" required value="1" min="1" max="10">
    ...
    addQuestionForm.addEventListener('submit', async (event) => {
        ...
        answerOptions.forEach((option, index) => {
            ...
            if (isNaN(score) || score < 1 || score > 10) {
                alert(`Lütfen ${index + 1}. cevap için 1 ile 10 arasında geçerli bir puan girin.`);
                allAnswersValid = false;
                return; // forEach'tan çıkmak için değil, callback'ten çıkmak için
            }
    ...
    function openEditQuestionForm(question, testId, placeholderElement) {
        ...
        function addAnswerOptionForDynamicEdit(text = '', score = 1, answerId = null) {
            ...
            answerDiv.innerHTML = `
                ...
                <input type="number" name="edit_score_${localEditAnswerOptionCount}" placeholder="Puan (1-10)" required value="${score}" min="1" max="10">
        ...
        dynamicForm.addEventListener('submit', async (event) => {
            ...
            answerOptions.forEach((option, index) => {
                ...
                if (isNaN(ansScore) || ansScore < 1 || ansScore > 10) {
                    alert(`Lütfen ${index + 1}. cevap için 1 ile 10 arasında geçerli bir puan girin.`);
                    allAnswersValid = false; return;
                }
