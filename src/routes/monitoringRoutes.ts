...
router.get('/avatar-stats', authMiddleware, adminMiddleware, async (req: Request, res: Response) => {
  try {
...
async function getPerformanceMetrics() {
  try {
    ...
    const avatarMetrics = await pool.query(`
      SELECT 
...
function calculateMonthlyCost(totalSizeBytes: number) {
...
function calculateStorageEfficiency(stats: any) {
...
function calculateDeduplicationSavings(stats: any) {
...
async function getAverageResponseTime(timeframe: string) {
  try {
...
async function getErrorRate(timeframe: string) {
  try {
...
async function getUploadMetrics(timeframe: string) {
  try {
    let timeCondition = '';
...
async function getCacheMetrics(timeframe: string) {
  try {
...
async function getDatabaseConnections() {
  const result = await pool.query('SELECT COUNT(*) FROM pg_stat_activity WHERE datname = current_database()');
...
async function getTrendData() {
  try {
    ...
    const totalStorage = await pool.query('SELECT SUM(file_size) as total FROM avatar_files');
