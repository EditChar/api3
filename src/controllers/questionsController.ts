...
export const addQuestionToTest = async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    ...
    const questionResult = await client.query(
      'INSERT INTO questions (test_id, question_text) VALUES ($1, $2) RETURNING id, test_id, question_text, created_at',
    ...
    if (newQuestion.id) {
      for (const ans of answers) {
        const answerResult = await client.query(
          'INSERT INTO answers (question_id, answer_text, score) VALUES ($1, $2, $3) RETURNING id, question_id, answer_text, score, created_at',
          [newQuestion.id, ans.answer_text, ans.score]
        );
...
export const getQuestionsForTest = async (req: AuthenticatedRequest, res: Response) => {
...
export const updateQuestion = async (req: AuthenticatedRequest, res: Response) => {
  const { questionId } = req.params;
  const { question_text, answers } : { question_text: string, answers: Array<{id?: number, answer_text: string, score: number }> } = req.body;
  ...
  try {
    ...
    const updatedQuestionResult = await client.query(
      'UPDATE questions SET question_text = $1 WHERE id = $2 RETURNING id, test_id, question_text, created_at',
    ...
    const updatedQuestion: Question = updatedQuestionResult.rows[0];

    const incomingAnswerIds: number[] = [];
    ...
    for (const ans of answers) {
      if (ans.id) { // Mevcut cevap, güncelle
        const updatedAnswerResult = await client.query(
          'UPDATE answers SET answer_text = $1, score = $2 WHERE id = $3 AND question_id = $4 RETURNING id, question_id, answer_text, score, created_at',
          [ans.answer_text, ans.score, ans.id, questionId]
        );
        ...
        const insertedAnswerResult = await client.query(
          'INSERT INTO answers (question_id, answer_text, score) VALUES ($1, $2, $3) RETURNING id, question_id, answer_text, score, created_at',
          [questionId, ans.answer_text, ans.score]
        );
        finalAnswers.push(insertedAnswerResult.rows[0]);
        if(insertedAnswerResult.rows[0].id) incomingAnswerIds.push(insertedAnswerResult.rows[0].id);
      }
    }
    ...
    if (incomingAnswerIds.length > 0) {
        ...
        await client.query(
            `DELETE FROM answers WHERE question_id = $1 AND id NOT IN (${placeholders})`,
...
export const deleteQuestion = async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    ...
    const deleteUserResponsesResult = await client.query(
        'DELETE FROM user_question_responses WHERE answer_id IN (SELECT id FROM answers WHERE question_id = $1)',
        [questionId]
    );
