 ...
 */
function calculateCompatibility(scoreDifference: number): number {
 ...
 */
export const checkMatchEligibility = async (req: AuthenticatedRequest, res: Response) => {
  const userId = req.user?.id;
  ...
  try {
    ...
    const userTestStatusResult = await pool.query(`
      SELECT 
