...
import { ChatRoom, ChatRoomWithUsers } from '../models/ChatRoom';
import { Message, MessageWithSender } from '../models/Message';
...
export const getChatRooms = async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    ...
    const rooms = await Promise.all(rows.map(async (row) => {
      ...
      try {
        const messages = await redisClient.getMessages(row.id, 1, 0);
...
export const getChatRoom = async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    ...
    try {
      ...
      await kafkaService.sendUserEvent(userEvent);
    } catch (kafkaError) {
...
export const enterChatRoom = async (req: AuthenticatedRequest, res: Response) => {
  const userId = req.user?.id;
  const roomId = req.params.roomId;
...
export const getMessages = async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    ...
    const roomResult = await pool.query(
      'SELECT * FROM chats WHERE id = $1 AND (user1_id = $2 OR user2_id = $2) AND status = $3',
    ...
    try {
      ...
      console.log(`📊 Redis client connected: ${redisClient.isConnected()}`);
      console.log(`📊 Redis using in-memory: ${redisClient.isUsingInMemory()}`);
      ...
      const rawMessages = await redisClient.lrange(testKey, 0, 2);
      console.log(`🔧 Debug: Raw Redis lrange result: ${JSON.stringify(rawMessages)}`);
      
      redisMessages = await redisClient.getMessages(roomId, limit, offset);
      ...
      if (redisMessages.length > 0) {
        ...
        console.log(`🔧 Redis cluster status: ${redisClient.getCluster() ? 'Connected' : 'Disconnected'}`);
      }
    } catch (redisError) {
    ...
    const messages: Message[] = redisMessages.map((redisMsg, index) => ({
...
export const sendMessage = async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    ...
    const roomResult = await pool.query(
      'SELECT user1_id, user2_id FROM chats WHERE id = $1 AND (user1_id = $2 OR user2_id = $2) AND status = $3',
      [roomId, senderId, 'active']
    );
    ...
    try {
      await kafkaService.sendChatMessage(chatMessage);
      ...
      try {
        await redisClient.saveMessage(roomId, redisMessage);
...
export const markChatRoomAsRead = async (req: AuthenticatedRequest, res: Response) => {
  const userId = req.user?.id;
  const roomId = req.params.roomId;
...
export const sendTypingIndicator = async (req: AuthenticatedRequest, res: Response) => {
  const userId = req.user?.id;
...
export const endRoom = async (req: AuthenticatedRequest, res: Response) => {
  const userId = req.user?.id;
  const { roomId } = req.params;
  ...
  try {
    ...
    try {
      await redisClient.expire(`chat_room:${roomId}:messages`, 3600); // 1 hour retention after end
    } catch (redisError) {
...
export const updateRoom = async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    // Room'un varlığını kontrol et
    const roomResult = await pool.query(
      'SELECT user1_id, user2_id, expires_at FROM chats WHERE id = $1 AND (user1_id = $2 OR user2_id = $2) AND status = $3',
      [roomId, userId, 'active']
    );
    ...
    if (expires_at) {
      await pool.query(
        'UPDATE chats SET expires_at = $1 WHERE id = $2',
        [expires_at, roomId]
      );
...
export const createChatRoom = async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    ...
    const result = await pool.query(
      'INSERT INTO chats (id, user1_id, user2_id, created_at, expires_at, status, last_message_at) VALUES ($1, $2, $3, NOW(), NOW() + INTERVAL \'7 days\', $4, NOW()) RETURNING *',
      [roomId, userId, otherUserId, 'active']
    );
...
export const getExpiredChatRooms = async (req: AuthenticatedRequest, res: Response) => {
  const userId = req.user?.id;
...
export const deleteExpiredChatRooms = async (req: AuthenticatedRequest, res: Response) => {
  const userId = req.user?.id;
...
export const getActiveChat = async (req: AuthenticatedRequest, res: Response) => {
  const userId = req.user?.id;
  ...
  try {
    ...
    const activeChat: ChatRoomWithUsers = {
      id: row.id,
      user1_id: row.user1_id,
      user2_id: row.user2_id,
      created_at: row.created_at,
