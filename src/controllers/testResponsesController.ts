...
import { UserTestResponse } from '../models/UserTestResponse';
import { UserScore } from '../models/UserScore';
import { UserQuestionResponse } from '../models/UserQuestionResponse';
import { AuthenticatedRequest } from '../middlewares/authMiddleware';

// Kullanıcının teste verdiği cevapları kaydetme ve puanlama
export const submitTestResponse = async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    ...
    const questionResponses: UserQuestionResponse[] = [];

    // Her cevap için puanı hesapla ve kaydet
    for (const response of responses) {
      // Cevabın puanını al
      const scoreQuery = await client.query(
        'SELECT score FROM answers WHERE id = $1 AND question_id = $2',
      ...
      const questionResponseResult = await client.query(
        'INSERT INTO user_question_responses (user_id, question_id, answer_id, response_score) VALUES ($1, $2, $3, $4) RETURNING *',
        [userId, response.question_id, response.answer_id, answerScore]
      );
    ...
    const testResponseResult = await client.query(
      'INSERT INTO user_test_responses (user_id, test_id, test_score) VALUES ($1, $2, $3) RETURNING *',
      [userId, testId, totalTestScore]
    );

    const testResponse: UserTestResponse = testResponseResult.rows[0];
...
const updateUserScore = async (client: any, userId: number) => {
  // Kullanıcının toplam puanını ve tamamladığı test sayısını hesapla
  const userStatsQuery = await client.query(
    'SELECT COUNT(*) as completed_tests, COALESCE(SUM(test_score), 0) as total_score FROM user_test_responses WHERE user_id = $1',
    [userId]
  );
  ...
  await client.query(
    ...
    [userId, total_score, completed_tests]
  );
};

// Kullanıcının test sonuçlarını listeleme
export const getUserTestResponses = async (req: AuthenticatedRequest, res: Response) => {
  const userId = req.user?.id;
  ...
  try {
    ...
    const countResult = await pool.query(
      'SELECT COUNT(*) FROM user_test_responses WHERE user_id = $1',
      [userId]
    );
...
export const getUserScore = async (req: AuthenticatedRequest, res: Response) => {
  const userId = req.user?.id;
...
export const getTestResponseDetails = async (req: AuthenticatedRequest, res: Response) => {
  const { testResponseId } = req.params;
  const userId = req.user?.id;

  try {
    // Test yanıtının varlığını ve kullanıcıya ait olduğunu kontrol et
    const testResponseResult = await pool.query(
      `SELECT utr.*, t.title as test_title 
       FROM user_test_responses utr
       JOIN tests t ON utr.test_id = t.id
       WHERE utr.id = $1 AND utr.user_id = $2`,
    ...
    const questionResponsesResult = await pool.query(
      `SELECT uqr.*, q.question_text, a.answer_text
...
export const getCompletedTests = async (req: AuthenticatedRequest, res: Response) => {
  const userId = req.user?.id;
  const { page = 1, limit = 10 } = req.query;
...
export const checkTestCompletion = async (req: AuthenticatedRequest, res: Response) => {
  const { testId } = req.params;
  const userId = req.user?.id;

  try {
    const result = await pool.query(
      'SELECT id, test_score, completed_at FROM user_test_responses WHERE user_id = $1 AND test_id = $2',
      [userId, testId]
    );
