...
import { AuthenticatedRequest } from '../middlewares/authMiddleware'; // req.user için
...
export const createTest = async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    const result = await pool.query(
      'INSERT INTO tests (title, description, created_by) VALUES ($1, $2, $3) RETURNING id, title, description, created_by, created_at',
      [title, description || null, userId]
    );
    const newTest: Test = result.rows[0];
...
export const getAllTests = async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    ...
    const tests: Test[] = result.rows.map(row => ({
        ...row,
...
export const getTestById = async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    const testResult = await pool.query('SELECT t.*, u.username as creator_username FROM tests t LEFT JOIN users u ON t.created_by = u.id WHERE t.id = $1', [testId]);
...
export const updateTest = async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    ...
    const checkOwnership = await pool.query('SELECT created_by FROM tests WHERE id = $1', [testId]);
    ...
    const result = await pool.query(
      'UPDATE tests SET title = $1, description = $2 WHERE id = $3 RETURNING id, title, description, created_by, created_at',
      [title, description || null, testId]
    );
...
export const toggleTestVisibility = async (req: AuthenticatedRequest, res: Response) => {
  const { testId } = req.params;
  const userId = req.user?.id;

  try {
    // Mevcut görünürlük durumunu al
    const currentTest = await pool.query('SELECT is_visible FROM tests WHERE id = $1', [testId]);
    ...
    const result = await pool.query(
      'UPDATE tests SET is_visible = $1 WHERE id = $2 RETURNING id, title, is_visible',
      [newVisibility, testId]
    );
...
export const deleteTest = async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    ...
    if (questionIds.length > 0) {
      ...
      const deleteUserQuestionResponsesResult = await client.query(
        `DELETE FROM user_question_responses WHERE question_id IN (${questionIdPlaceholders})`,
        questionIds
      );
      ...
      const deleteAnswersResult = await client.query(
        `DELETE FROM answers WHERE question_id IN (${questionIdPlaceholders})`,
        questionIds
      );
